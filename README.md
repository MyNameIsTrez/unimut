...
You temporarily mark a region of a source file with `unimut start` and `unimut stop` code comments (these markers should not be committed), and pass `unimut` a command that builds and tests your project. It then tries every mutation of the marked code to see which ones your test suite fails to notice.
